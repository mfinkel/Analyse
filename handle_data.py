...
class POLDIData(Data):
    def __init__(self, sample_diameter, odf_phase_1_file=None, odf_phase_2_file=None):
        super(POLDIData, self).__init__(sample_diameter, odf_phase_1_file, odf_phase_2_file)
        self.data_dic_raw = {}  # a dictionary containing the raw data, the key is the applied force, the values
        # are lists of all measured orientation angles with the measured two_theta, intens, error data
        self.data_dic_phases = {}  # dictionary containing dictionaries of all peaks of the different phases
        # data_dic_phases key=phase, val=dic containing all reflexes (key = force)
    ...
    @staticmethod
    def pars_poldi_ref_res_file(filename):
        data = open(filename, "r")
        lines = data.readlines()
        pattern_peak = re.compile(r'(Peak *)([0-9]+)')
        pattern_miller = re.compile(r'(\d) *\((\d)(\d)(\d)\)')
        pattern_position = re.compile(r'(\d+.{1}\d+)')
        hkl_d_derr = []
        ph, h, k, l, d, derr = [], [], [], [], [], []
        for line in lines:
            if "Peak " in line:
                for (Peak, nr) in re.findall(pattern_peak, line):
                    pass
            if "Miller-index hkl" in line:
                for (p, h_, k_, l_) in re.findall(pattern_miller, line):
                    ph.append(int(p))
                    h.append(h_)
                    k.append(k_)
                    l.append(k_)
            if "position (d)" in line:
                for i in re.findall(pattern_position, line):
                    d.append(float(i))
            if "deltad/d" in line:
                for i in re.findall(pattern_position, line):
                    derr.append(float(i)*np.power(10.,-3))

        for i in xrange(len(ph)):
            hkl_d_derr.append([ph[1], ])
...
class DataContainer(object):
    def __init__(self):
        ...
        # as key and the list [hkl_phi_psi, epsilon_sigma] as val's.
